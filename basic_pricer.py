...
def bs_price(spot, strike, vol, expiry, is_call = True,
             rate_dom = 0.00, rate_for = 0.00, is_prem_for = False):
    ...
    fwd = spot * df(rate_dom,expiry) / df(rate_for,expiry)
    d1 = (log(fwd/strike) + vol**2*expiry/2) / (vol*sqrt(expiry))
    d2 = d1 - vol*sqrt(expiry)
    if is_prem_for:
        div_by = strike/df(rate_dom,expiry)
    else:
        div_by = spot/df(rate_for,expiry)
    if is_call:
        return (fwd*phi(d1) - strike*phi(d2))/div_by
    else:
        return (strike*phi(-d2) - fwd*phi(-d1))/div_by

def bs_call(spot, strike, vol, expiry, rate_dom = 0.00,
            rate_for = 0.00, is_prem_for = False):
    ' wrapper for price of call bs formula '
    return bs_price(spot, strike, vol, expiry, True, rate_dom, rate_for, is_prem_for)

def bs_put(spot, strike, vol, expiry, rate_dom = 0.00,
            rate_for = 0.00, is_prem_for = False):
    ' wrapper for price of put bs formula '
    return bs_price(spot, strike, vol, expiry, False, rate_dom, rate_for, is_prem_for)    

def bs_delta(spot, strike, vol, expiry, is_call = True,
             rate_dom = 0.00, rate_for = 0.00):
...
def bs_strike(spot, delta, vol, expiry, is_call = True,
              rate_dom = 0.00, rate_for = 0.00, is_prem_for = False):
    fwd = spot * df(rate_dom,expiry) / df(rate_for,expiry)
    if is_call:
        return fwd/exp(norm_CDF_inv(delta)*vol*sqrt(expiry) - vol**2*expiry/2)
    else:
        return fwd/exp(norm_CDF_inv(-delta + 1)*vol*sqrt(expiry) - vol**2*expiry/2)
    

def implied_vol_bs(price, spot, strike, expiry, is_call = True, rate_dom = 0.00,
                   rate_for = 0.00, is_prem_for = False, vol_high = 2.00):
    ...
    while ((high_vol - low_vol) >= epsilon) and i < 100:
        ...
        if bs_price(spot, strike, mid_vol, expiry, is_call, rate_dom, rate_for, is_prem_for) > price:
            high_vol = mid_vol
        else:
            low_vol = mid_vol
        i += 1
...
def realized_vol(prices, start, end):
