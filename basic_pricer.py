...
def bs_price(spot, strike, vol, expiry, is_call = True,
             ir_d = 0.00, ir_f = 0.00, is_prem_for = False):
...
def bs_call(spot, strike, vol, expiry, ir_d = 0.00,
...
def implied_vol_bs(price, spot, strike, expiry, is_call = True, ir_d = 0.00,
    ...
    while ((high_vol - low_vol) >= epsilon) and i < 100:
        ...
        if bs_price(spot, strike, mid_vol, expiry, is_call, ir_d, ir_f, is_prem_for) > price:
            high_vol = mid_vol
        else:
            low_vol = mid_vol
        i += 1
...
def implied_vol_str(price, spot, strike, expiry, ir_d = 0.00,
                   ir_f = 0.00, is_prem_for = False, vol_high = 2.00):
    ...
    while ((high_vol - low_vol) >= epsilon) and i < 100:
        mid_vol = (low_vol + high_vol)/ 2.0
        if bs_str(spot, strike, mid_vol, expiry, ir_d, ir_f, is_prem_for) > price:
            high_vol = mid_vol
        else:
            low_vol = mid_vol
        i += 1
